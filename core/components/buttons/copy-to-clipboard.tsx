...
const CopyToClipboardButton = (props: { text: string; title?: string }) => {
    ...
    const tickVariants = {
        pressed: (isChecked: boolean) => ({
            pathLength: isChecked ? 0.85 : 0.05,
        }),
    ...
    return (
        <Button
            ...
            variant="outline"
            size="icon"
            className="hover:shadow-[0_0_13px_rgba(59,130,246,1)] hover:border-[rgba(59,130,246,1)] transition-all duration-300 ease-in-out"
        >
            <motion.svg
                initial="idle"
                whileHover="hover"
