...
// Generate dynamic metadata for case study
export async function generateImageMetadata({
  ...
  try {
    const post = await getFileBySlug(params.slug, 'case-studies');
    ...
    return [
      {
        id: 'og',
        alt: `Case Study | ${siteConfig.title}`,
        size,
        contentType,
      },
    ];
...
export default async function Image({ params }: { params: { slug: string } }) {
  let post;
  try {
    post = await getFileBySlug(params.slug, 'case-studies');
  } catch (error) {
    ...
    return new ImageResponse(
      (
        ...
        >
          <h1 style={{ fontSize: 60 }}>Case Study</h1>
        </div>
      ),
      { ...size }
    );
      ...
      >
        ...
        <div style={{
          ...
          <div style={{
            ...
            {/* Case Studies section identifier */}
            <div style={{
              width: '220px',
          ...
          <div style={{
            ...
            {`${siteConfig.url.replace('https://', '')}/case-studies/${params.slug}`}
          </div>
        </div>
      </div>
    ),
